...
function FloatingParticle({ delay, x, size }: FloatingParticleProps) {
  return (
    <motion.div
      ...
      transition={{ duration: 10 + size, delay, repeat: Infinity, ease: "linear" }}
    />
  );
}
...
function FeatureGrid({ items, variant }: { items: FeatureItem[]; variant: "steps" | "highlights" }) {
  return (
    <div className={variant === "steps" ? "grid gap-8 sm:grid-cols-2 lg:grid-cols-4" : "grid gap-6 sm:grid-cols-2 lg:grid-cols-4"}>
      {items.map((item, index) => (
        <motion.div
          ...
          whileHover={variant === "steps" ? { y: -6, transition: { duration: 0.2 } } : { scale: 1.03, transition: { duration: 0.2 } }}
          className={
            variant === "steps"
              ? "clip-angle border border-border gradient-card p-6 text-center cursor-default group"
              : "border border-border bg-card/50 rounded-lg p-6 text-center cursor-default group"
          }
        >
          {variant === "steps" ? (
            <>
            ...
            <item.icon className="mx-auto h-8 w-8 text-primary/70 mb-3 transition-colors group-hover:text-primary" />
          )}
          <h3 className="font-display text-sm font-bold tracking-wider text-foreground">{item.title}</h3>
...
function Index() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActivityIndex((currentIndex) => (currentIndex + 1) % ACTIVITIES.length);
    }, 3000);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMatchStep((currentStep) => (currentStep + 1) % 2);
    }, 4000);
    return () => clearInterval(interval);
  }, []);
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative flex min-h-[75vh] items-center justify-center overflow-hidden pt-16">
        ...
        {PARTICLES.map((particle) => (
          <FloatingParticle key={particle.delay} {...particle} />
        ))}
        ...
        <div className="relative z-10 container text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.6 }} className="mt-10">
            <Link
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="mx-auto mt-8 h-8 max-w-sm overflow-hidden">
      ...
      <section className="container py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="font-display text-sm tracking-[0.3em] text-muted-foreground">O QUE É O GAME MATCHING?</h2>
      ...
      <section className="container py-16">
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center font-display text-sm tracking-[0.3em] text-muted-foreground mb-10">
          PREVIEW DE MATCH
        </motion.h2>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="mx-auto max-w-lg clip-angle border border-border gradient-card p-8">
          <div className="flex items-center justify-between">
            <motion.div className="text-center flex-1" animate={{ x: matchStep === 1 ? 10 : 0 }} transition={{ type: "spring", stiffness: 200 }}>
              <div className="mx-auto h-14 w-14 rounded-full border-2 border-primary/40 bg-muted flex items-center justify-center font-display text-lg font-bold text-primary">SK</div>
            ...
            <div className="flex-shrink-0 mx-4 text-center">
              <AnimatePresence mode="wait">
                {matchStep === 0 ? (
                  <motion.div key="searching" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }}>
                    <motion.div className="h-12 w-12 mx-auto rounded-full border-2 border-primary/30 border-t-primary" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} />
                    <p className="mt-2 font-display text-xs tracking-widest text-muted-foreground">BUSCANDO...</p>
                  </motion.div>
                ) : (
                  <motion.div key="found" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} transition={{ type: "spring", stiffness: 300 }}>
                    <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 0.6, repeat: 2 }}>
                      <CheckCircle2 className="mx-auto h-10 w-10 text-primary" />
                    </motion.div>
                    <p className="mt-2 font-display text-xs font-bold tracking-wider text-primary text-glow-primary">MATCH ENCONTRADO!</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <motion.div className="text-center flex-1" animate={{ x: matchStep === 1 ? -10 : 0 }} transition={{ type: "spring", stiffness: 200 }}>
              <div className="mx-auto h-14 w-14 rounded-full border-2 border-secondary/40 bg-muted flex items-center justify-center font-display text-lg font-bold text-secondary">IF</div>
      ...
      <section className="container py-16">
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center font-display text-sm tracking-[0.3em] text-muted-foreground mb-12">
          COMO FUNCIONA
        </motion.h2>
        <FeatureGrid items={MATCH_STEPS} variant="steps" />
      </section>
      ...
      <section className="container py-16">
        ...
        <FeatureGrid items={HIGHLIGHTS} variant="highlights" />
      </section>
      ...
      <section className="container py-16">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 max-w-4xl mx-auto">
          {MOCK_PLAYERS.map((player, index) => (
            ...
            >
              <div className="h-10 w-10 flex-shrink-0 rounded-full border border-primary/30 bg-muted flex items-center justify-center font-display text-sm font-bold text-primary">{player.avatar}</div>
      ...
      <section className="container py-20">
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center font-display text-sm tracking-[0.3em] text-muted-foreground">
          NOSSO JOGO
        </motion.h2>
        <div className="mt-8 max-w-lg mx-auto">
          ...
          >
            <div className="relative h-64 overflow-hidden">
              <img src={lolHero} alt="League of Legends" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
              <div className="absolute inset-0 bg-gradient-to-t from-background via-background/50 to-transparent" />
            </div>
            <div className="relative p-6">
              <h3 className="font-display text-2xl font-bold tracking-wider text-primary">LEAGUE OF LEGENDS</h3>
              <p className="mt-2 text-sm text-muted-foreground">Encontre seu duo partner. Subam juntos no ranking.</p>
              <div className="mt-4 flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-primary animate-pulse-glow" />
                <span className="font-display text-xs tracking-widest text-muted-foreground">FILA DISPONÍVEL</span>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="container py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="font-display text-3xl font-bold tracking-wider text-foreground md:text-4xl">
            ENCONTRE SEU <span className="text-primary text-glow-primary">PRÓXIMO TEAMMATE</span>
          </h2>
          <div className="mt-8">
            <Link to={user ? "/matchmaking" : "/auth"} className="group clip-angle-sm relative inline-block bg-primary px-12 py-4 font-display text-sm font-semibold tracking-wider text-primary-foreground transition-all hover:box-glow-primary hover:scale-105 active:scale-95">
