...
const OnboardingModal = ({ userId, defaultUsername, onComplete }: OnboardingModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim()) { toast.error("Nome de usuário é obrigatório"); return; }
    if (!riotId.trim()) { toast.error("Riot ID é obrigatório"); return; }
    ...
    try {
      ...
      if (existing) { toast.error("Este nome de usuário já está em uso. Escolha outro."); setLoading(false); return; }

      if (riotId) {
        const { data: existingRiot } = await supabase
          .from("profiles").select("id").eq("riot_id", riotId.trim()).neq("user_id", userId).maybeSingle();
        if (existingRiot) { toast.error("Este Riot ID já está vinculado a outra conta."); setLoading(false); return; }
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/90 backdrop-blur-sm">
      ...
      >
        <div className="flex flex-col items-center gap-3 mb-6">
          <img src={logo} alt="Game Matching" className="h-14" />
